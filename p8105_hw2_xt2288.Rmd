...
In the import process, the first line is skipped to make the second line as the header. The dataset has `r nrow(mci_biomarker_df)` observations and `r ncol(mci_biomarker_df)` variables.
...
The combined dataset has `r nrow(mci_combine_df)` observations and `r ncol(mci_combine_df)` variables.
